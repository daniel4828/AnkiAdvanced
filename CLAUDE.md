...
| R13 | **给 Daniel 的终端命令一律打包成临时脚本** | 让 Daniel 复制粘贴多行命令经常因格式问题（换行/引号）失败。凡需要他在终端执行的操作，写成一个简短的临时脚本文件（含 `echo` 进度提示），让他 `bash xxx.sh` 运行，用完删除 |
...
├── podcast.py              # 播客爬虫（#479）：YouTube RSS 发现新视频、yt-dlp 拉中文转录（无字幕时 NotebookLM 免费转录主力 + Whisper 付费保底，#486/#485）、AI 德语摘要+HSK生词、邮件通知
├── tts.py                 # edge-tts 封装
...
GET/PUT /api/podcast/config                           → 读/改设置（channel_url/email_to/detail_level/enabled/transcriber/whisper_max_minutes；whisper_fallback、whisper_title_filter 已废弃但兼容）
...
| `YT_DLP_COOKIES` | `data/yt_cookies.txt` | YouTube Cookie 文件（#491）——服务器 IP 被 YouTube 要求登录验证，所有 yt-dlp 调用都需要；文件不存在时自动跳过（本地开发不需要）。导出方法见 `scripts/README.md` |
...
- 允许的外部依赖：`fastapi`、`uvicorn`、`anthropic`、`openai`、`edge-tts`、`pyyaml`、`python-multipart`、`deep-translator`（可选）、`jieba`、`pypinyin`、`yt-dlp`（播客转录下载，#479）、`notebooklm-py`（播客 NotebookLM 免费转录，#486，非官方库，凭据文件一次性从本地拷到服务器，见 `scripts/README.md`）。新增依赖必须同步更新 `requirements.txt`。播客无字幕转录链（#486 NotebookLM / #485 Whisper 保底）另需系统级 `ffmpeg`（`apt install ffmpeg`，不是 Python 依赖，缺失时该功能自动跳过）
