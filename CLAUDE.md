...
├── podcast.py              # 播客爬虫（#479）：YouTube RSS 发现新视频、yt-dlp 拉中文转录、AI 德语摘要+HSK生词、邮件通知
├── tts.py                 # edge-tts 封装
...
GET/PUT /api/podcast/config                           → 读/改设置（channel_url/email_to/detail_level/enabled）
...
- 允许的外部依赖：`fastapi`、`uvicorn`、`anthropic`、`openai`、`edge-tts`、`pyyaml`、`python-multipart`、`deep-translator`（可选）、`jieba`、`pypinyin`、`yt-dlp`（播客转录下载，#479）。新增依赖必须同步更新 `requirements.txt`
