...
- **排查线上故障第一步永远是看日志**：`journalctl -u ankiadvanced` —— 播客卡死（#565/#566）、早晨预生成失败（#517）这些事故，日志里都写着原因，靠猜要多花几小时
...
- 🔴 **生产库上的写操作要先问 Daniel**：`/home/anki/AnkiAdvanced/data/srs.db` 是唯一真库。只读查询（`SELECT`）随便跑；`UPDATE`/`DELETE`/迁移必须先说清楚要改什么、影响多少行，得到同意再动。备份是每 6 小时一次，最坏能丢 6 小时
