...
│   ├── review.py          # /api/review, /api/today
│   ├── story.py           # /api/story
│   └── utils.py           # 共用工具（DISABLE_AI, leaf_ids）
├── data/
...
- 除以下库外不引入外部依赖（yīlài - dependencies）：`fastapi`、`uvicorn`、`anthropic`、`openai`、`edge-tts`、`pyyaml`、`python-multipart`
