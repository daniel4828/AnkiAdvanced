...
├── podcast.py              # 播客爬虫（#479）：YouTube RSS 发现新视频、yt-dlp 拉中文转录（无字幕时 Whisper 音频转录回退，#485）、AI 德语摘要+HSK生词、邮件通知
├── tts.py                 # edge-tts 封装
...
GET/PUT /api/podcast/config                           → 读/改设置（channel_url/email_to/detail_level/enabled/whisper_fallback）
...
- 允许的外部依赖：`fastapi`、`uvicorn`、`anthropic`、`openai`、`edge-tts`、`pyyaml`、`python-multipart`、`deep-translator`（可选）、`jieba`、`pypinyin`、`yt-dlp`（播客转录下载，#479）。新增依赖必须同步更新 `requirements.txt`。播客 Whisper 音频回退（#485）另需系统级 `ffmpeg`（`apt install ffmpeg`，不是 Python 依赖，缺失时该功能自动跳过）
