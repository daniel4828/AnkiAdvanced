...
2. ...""",
    ...
    # 现在硬性规则是"专有名词或硬数据至少有一类"，并要求先挑出与词数相同的
    # 互不相同的事实、一条只用一次；A 档为首选档，且允许换角度重述事实来容纳目标词。
...
【硬性事实规则，最重要】
每一句话都必须落在素材里的一条具体事实上，并且至少包含下面两类信息中的一类
（两类都有最好）：
  ① 素材里的专有名词：人名、公司名、品牌名、产品名、机构名、地名
  ② 素材里的硬数据：年份、日期、数量、金额、百分比、名次、时长、年龄
两类都没有的句子一律不合格，必须重写。专有名词和数字必须照抄素材里的原文，
绝对不许自己编造或改动。

【事实分配规则】
- 动笔之前，先从素材里挑出若干条互不相同的事实，数量与目标词数量相同；
  这些事实要分散在素材的开头、中间和结尾，素材里的每一个话题都至少覆盖一条
- 一条事实只能用在一个句子里。两句话不许讲同一件事——换个说法重复也算同一件事
- 严禁所有句子都挤在同一个话题上，也不要只用素材开头的内容
- 属于同一话题的句子尽量排在一起，让整篇按话题一块一块地读下来，不要来回跳

【就近三档原则】
每个目标词都先尽力写成 A 档，只有确实写不自然时才往下降一档：
  A档 事实句【首选，大多数句子都应该是 A 档】：把挑好的那条事实写成句子，
     把目标词自然地嵌进去。为了容纳目标词，允许换个角度重述这条事实
     （从人物、时间、原因、影响等角度说），但事实本身、数字和名字不能变。
     例：2021年，张一鸣辞去了字节跳动CEO的职位。
  B档 评论句：某人对这条事实的反应、评价或愿望，句中仍要带上该事实的名字或数字。
     例：我真羡慕那些能天天吃到3000元一斤的和牛的人。
  C档 场景句：一个围绕这条事实展开的日常小场景，句中仍要带上该事实的名字或数字。
     例：他在抖音上刷视频，顺便就下了单。

【写作步骤】（每个词按顺序思考，把结论写进 reasoning_zh）
1. 先从上面挑好的事实清单里选一条还没用过的事实，并记下它属于哪个话题
2. 记下这条事实里的锚点：专有名词和硬数据（年份、数字、金额等）
3. 然后定档位（A/B/C）——先试 A 档，写不自然再降档
4. 最后写 sentence_zh：把这条事实写成一句自然的中文，锚点照抄，目标词自然嵌入

【其他规则】
- 每句恰好包含一个目标词汇，一个词都不能漏，也不要在一句里塞进两个目标词
- 除专有名词、数字（年份、金额、百分比、单位等）和目标词外，只用 HSK 1-{max_hsk} 的词汇
- 每句 10 到 30 个字，用自然的中文口语，标点用中文标点
- C档允许虚构日常细节（谁在排队、谁在点菜），但绝不可以编造素材里没有的数字、
  结论或事实性主张，也不可以与素材矛盾
- 严禁空洞句子，例如"这一集很有趣。""他说了很多。"；也严禁把事实说得含糊，
  素材里给了具体数字或名字就必须写进句子，不许写成"很多人""好几年""一家公司"
- 只用简体中文，绝对不要出现繁体字；不要使用markdown格式
- 不要给目标词加引号、括号或任何标记，直接写进句子里
- target_word 是该句包含的目标词汇原文

reasoning_zh 的规则：
- 用中文写1-2句，先写"事实："加上这句用的那条事实（含其中的名字和数字），
  再说明它属于哪个话题、选了哪一档
- 这条事实必须与其它句子用的事实不同——写出来正是为了让你自己检查有没有重复
