...
def get_decks():
    ...
    for deck in flat:
        ...
        deck["new_review_order_override"] = p.get("new_review_order_override")
...
def toggle_new_review_order(deck_id: int):
    ...
    if not deck:
        raise HTTPException(status_code=404, detail="Deck not found")
    preset = database.get_preset(deck["preset_id"])
    current = preset.get("new_review_order_override")
    cycle = {None: "mixed", "mixed": "reviews_first", "reviews_first": "new_first", "new_first": None}
    next_order = cycle.get(current, "mixed")
    database.update_preset(preset["id"], {"new_review_order_override": next_order})
